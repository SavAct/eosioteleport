  ...
  oracles: [
    {
      ...
      keys: generateAllKeys(
        "8940fbd7806ec09af7e1ceaf7ccac80e89eeeb1e85cee42f84c07b1d5a378100"
      ),
    },
    {
      eosio_name: "anneliese",
      keys: generateAllKeys(
        "fef658def915544a10ca323e6b4f33a6179043e1862a522c20837fb87869e668"
      ),
    },
    {
      eosio_name: "peterpeter12",
      keys: generateAllKeys(
        "579d5ba8a4423300b0c820930fdcc277bc805a29b94d026f35dec4b2e333820e"
      ),
    },
    {
      eosio_name: "helga",
      keys: generateAllKeys(
        "4040fbd7806ec09af7e1ceaf7ccac80e89eeeb1e85cee42f84c07b1d5a378110"
      ),
    },
    {
      eosio_name: "hans",
      keys: generateAllKeys(
        "7695855d7b7590c52a0af76ded6866724dea37f07e33e4545c9ea17a2740e529"
      ),
    },
  ],
};
...
function signWithTestSettings(logDataHex) {
  ...
  for (let i = 0; i < TestSettings.threshold; i++) {
    const sig = signWithKey(
      TestSettings.oracles[i].keys.ethPrivate,
      logDataKeccak
    );
    signatures.push(sig);
  }
...
contract("TeleportToken", (accounts) => {
  ...
  for (let i = 0; i < TestSettings.oracles.length; i++) {
    ...
    console.log(
      element.eosio_name,
      `${element.keys.ethAddress} ${element.keys.eosioPrivate}`
    );
  }
  ...
  describe("Total supply", function () {
    it("should be the same as initially defined", async () => {
      ...
      assert.equal(
        balance.valueOf(),
        TestSettings.Token.totalSupply,
        `Total supply is not ${TestSettings.Token.totalSupply}`
      );
    });
  });
  ...
  describe("Add chain", function () {
    before("Create chain parameters", async () => {
      ...
      addChain1Contract = getEosioAccountValue(
        TestSettings.EOSIO.contract_name
      );
      addChain2NetId =
        "0x" +
        "aa0aaab8997e1dfce58fbfac80cbbb8fecec7b99cf982a9444273cbc64c414aa";
      addChain2ShortNetId = addChain2NetId.substring(0, 10);
      addChain2Contract = getEosioAccountValue(addChain2ContractName);
    });
    it("should fail without authorization", async () => {
      await catchRevert(
        instance.addChain(addChain1NetId, addChain1Contract, 0, 0, {
          from: accounts[3],
        }),
        "Unauthorized chain registration"
      );
    });
    it("should succeed to add a chain", async () => {
      await instance.addChain(addChain1NetId, addChain1Contract, 0, 0, {
        from: accounts[0],
      });
    ...
    it("should fail to add the same chain with different contract and different chain id", async () => {
      await catchRevert(
        instance.addChain(addChain1NetId, addChain2Contract, 1, 0, {
          from: accounts[0],
        }),
        "Add the same chain net twice"
      );
    });
    it("should fail to add another chain with the chain id of this contract", async () => {
      await catchRevert(
        instance.addChain(addChain2NetId, addChain2Contract, 2, 0, {
          from: accounts[0],
        }),
        "Add a chain with the same id as this eth contract"
      );
    });
    it("should fail to add another chain with same chain id", async () => {
      await catchRevert(
        instance.addChain(addChain2NetId, addChain2Contract, 0, 0, {
          from: accounts[0],
        }),
        "Add the same chain id twice"
      );
    });
    it("should succeed to add another chain", async () => {
      await instance.addChain(addChain2NetId, addChain2Contract, 1, 0, {
        from: accounts[0],
      });
  ...
  describe("Remove chain", function () {
    it("should fail without authorization", async () => {
      await catchRevert(
        instance.rmChain(addChain2NetId, { from: accounts[3] }),
        "Unauthorized removing chain"
      );
    });
  ...
  describe("Register oracles", function () {
    it("should fail with unauthorized account", async () => {
      ...
      assert.equal(
        TestSettings.oracles.length > TestSettings.threshold,
        true,
        "Need one more oracle for testing than threshold"
      );

      // Check if someone else can register an oracle than owner
      await catchRevert(
        instance.regOracle(TestSettings.oracles[0].keys.ethAddress, {
          from: accounts[3],
        }),
        "Unauthorized oracle registration"
      );
    });
    it("should succeed to register several oracles", async () => {
      // Register all oracles
      for (let i = 0; i < TestSettings.oracles.length; i++) {
        await instance.regOracle(TestSettings.oracles[i].keys.ethAddress, {
          from: accounts[0],
        });
        assert.equal(
          await instance.oracles(TestSettings.oracles[i].keys.ethAddress),
          true,
          "Missing oracle" + i
        );
      }
    ...
    it("should succeed to remove an oracle", async () => {
      // Remove one oracle
      noOracle =
        TestSettings.oracles[TestSettings.oracles.length - 1].keys.ethAddress;
      await instance.unregOracle(noOracle, { from: accounts[0] });
      assert.equal(
        await instance.oracles(noOracle),
        false,
        "Oracle is still registered"
      );
    });
    it("should fail to reg an oracle with former oracle", async () => {
      await catchRevert(
        instance.regOracle(noOracle, { from: accounts[3] }),
        "Unauthorized registration of former oracle"
      );
    });
    it("should fail to reg an oracle twice", async () => {
      await catchRevert(
        ...
        "Double registration of the same oracle"
      );
    });
  });

  let tokenAmount1 = 500000;
  const fullToken1 = Math.round(
    tokenAmount1 / 10 ** TestSettings.Token.decimals
  );
  describe("Receive token from eosio chain", function () {
    ...
    before("Calc claim data", async () => {
      ...
      logAsset = `${fullToken1}.${"0".repeat(TestSettings.Token.decimals)} ${
        TestSettings.Token.symbol
      }`;
      logData = {
        id: 0, // uint64
    ...
    it("should have zero ballances at the beginning", async () => {
      assert.equal(
        await instance.balanceOf.call(accounts[0]).valueOf(),
        0,
        "Balance of account 0 is not 0"
      );
      assert.equal(
        await instance.balanceOf.call(accounts[1]).valueOf(),
        0,
        "Balance of account 1 is not 0"
      );
    });
    it("should fail to receive teleport with wrong signatures", async () => {
      ...
      for (let i = 0; i < TestSettings.threshold; i++) {
        const sig = signWithKey(
          TestSettings.oracles[i].keys.ethPrivate,
          ethUtil.keccak(Buffer(["1", "2", "3", "4"]))
        );
        falseSignatures.push(sig);
      }
      await catchRevert(
        instance.claim(logDataHex, falseSignatures, { from: accounts[1] }),
        "Claim with false signatures"
      );
    });
    it("should fail to receive teleport with inactive chain", async () => {
      await catchRevert(
        instance.claim(logDataHex2, signatures2, { from: accounts[2] }),
        "Claim with inactive chain"
      );
    });
    it("should succeed to claim a teleport", async () => {
      ...
      assert.equal(
        (await instance.balanceOf.call(accounts[2])).valueOf(),
        0,
        "Wrong account got funds"
      );
      assert.equal(
        (await instance.balanceOf.call(accounts[1])).valueOf(),
        fullToken1 * 10 ** TestSettings.Token.decimals,
        "Account 1 does not got the right amount of funds"
      );
    });
    it("should fail to claim the same teleport twice", async () => {
      await catchRevert(
        instance.claim(logDataHex, signatures, { from: accounts[2] }),
        "It is possible claim the same teleport twice"
      );
    });
    it("should fail to claim a teleport with already used id", async () => {
      await catchRevert(
        instance.claim(logDataHexTry, signaturesTry, { from: accounts[2] }),
        "It is possible to claim a teleport with an already used id"
      );
    });
    it("should fail to claim a teleport with an unknown chain id", async () => {
      ...
      await catchRevert(
        instance.claim(logDataHexTry, signaturesTry, { from: accounts[2] }),
        "It is possible to claim a teleport with an unknown chain id"
      );
    });
  ...
  describe("Send token within the chain", function () {
    ...
    it("should succeed to transfer tokens to yourself", async () => {
      ...
      assert.equal(
        (await instance.balanceOf.call(accounts[1])).valueOf(),
        tokenAmount1,
        "Token amount changed by sending to yourself"
      );
    });
    it("should fail to transfer with impossible amounts", async () => {
      await catchRevert(
        instance.transfer(accounts[2], tokenAmount1 + 3, { from: accounts[1] }),
        "It is possible to send more tokens than available for an account"
      );
      assert.equal(
        (await instance.balanceOf.call(accounts[2])).valueOf(),
        0,
        "It is possible to get more tokens from a sender than his balance is"
      );
      await catchRevert(
        instance.transfer(accounts[1], tokenAmount1, { from: accounts[3] }),
        "It is possible to send tokens without a balance"
      );
    });
    it("should succeed to send tokens to another account", async () => {
      ...
      assert.equal(
        (await instance.balanceOf.call(accounts[1])).valueOf(),
        tokenAmount1,
        "Token amount of sender account is not reduced"
      );
    ...
    it("should approve some tokens for another account", async () => {
      ...
      assert.equal(
        (await instance.balanceOf.call(accounts[3])).valueOf(),
        0,
        "Account should get rights not funds"
      );
      assert.equal(
        (await instance.balanceOf.call(accounts[1])).valueOf(),
        tokenAmount1,
    ...
    it("should succeed to send tokens by an approved account", async () => {
      await instance.transferFrom(accounts[1], accounts[4], sendAmount, {
        from: accounts[3],
      });
      ...
      assert.equal(
        (await instance.balanceOf.call(accounts[3])).valueOf(),
        0,
        "Account should not get funds by sending them to someone else"
      );
      assert.equal(
        (await instance.balanceOf.call(accounts[1])).valueOf(),
        tokenAmount1,
        "Owner has wrong balance amount"
      );
      assert.equal(
        (await instance.balanceOf.call(accounts[4])).valueOf(),
        sendAmount,
        "Receiver got not the right amount of funds"
      );
    });
    it("should fail to send approved tokens with wrong parameters", async () => {
      await catchRevert(
        instance.transferFrom(accounts[1], accounts[5], 1, {
          from: accounts[2],
        }),
        "It is possible to send tokens without the rights"
      );
      await catchRevert(
        instance.transferFrom(accounts[1], accounts[5], sendAmount + 3, {
          from: accounts[3],
        }),
        "It is possible to send more tokens than approved"
      );
    });
    it("should succeed to remove approve rights", async () => {
      ...
      assert.equal(
        (await instance.balanceOf.call(accounts[3])).valueOf(),
        0,
        "Account should not get funds by removing the approve"
      );
      assert.equal(
        (await instance.balanceOf.call(accounts[1])).valueOf(),
        tokenAmount1,
        "Owner has wrong balance amount after removing approve"
      );
      await catchRevert(
        instance.transferFrom(accounts[1], accounts[5], 1, {
          from: accounts[3],
        }),
        "Can not remove the right to transfer tokens"
      );
    });
  });
  let receiveChainId;
  ...
  describe("Teleport to eosio chain", function () {
    ...
    it("should succeed a teleport", async () => {
      await instance.teleport("fraugertrud", sendAmount, receiveChainId, {
        from: accounts[1],
      });
      assert.equal(
        (await instance.balanceOf.call(accounts[1])).valueOf(),
        tokenAmount1,
        "Balance of account got not reduced"
      );
      assert.equal(
        await instance.indexes(receiveChainId),
        1,
        `Wrong index fort teleports to chain ${receiveChainId}`
      );
    });
    it("should fail teleport without any balance", async () => {
      await catchRevert(
        instance.teleport("fraugertrud", sendAmount, receiveChainId, {
          from: accounts[6],
        }),
        "Can teleport without any balance"
      );
      sendAmount2 = tokenAmount1 / 10;
      tokenAmount1 -= 3 * sendAmount2;
    });
    it("should succeed further teleports with growing indexes", async () => {
      await instance.teleport(
        "frauerdbeere",
        sendAmount2,
        secondReceiveChainId,
        { from: accounts[1] }
      );
      await instance.teleport("frauerdbeere", sendAmount2, receiveChainId, {
        from: accounts[1],
      });
      await instance.teleport("frauerdbeere", sendAmount2, receiveChainId, {
        from: accounts[1],
      ...
      assert.equal(
        await instance.indexes(secondReceiveChainId),
        1,
        `Wrong index fort teleports to chain ${secondReceiveChainId}`
      );
      assert.equal(
        await instance.indexes(receiveChainId),
        3,
  ...
  describe("Check indexes by adding new chains", function () {
    ...
    it("should succeed to remove the first chain", async () => {
      ...
      assert.equal(
        (await instance.chains(addChain1ShortNetId)).active,
        false,
        "Removed first chain is still active"
      );
    });
    it("should fail to add the first chain with first used zero index", async () => {
      await catchRevert(
        ...
        "Can add chain with already used first index"
      );
    });
    ...
    it("should succeed a teleport from the first chain", async () => {
      await instance.teleport("frauerdbeere", sendAmount, receiveChainId, {
        from: accounts[1],
      });
      lastChainIndex++;
      assert.equal(
        await instance.indexes(receiveChainId),
        lastChainIndex,
        `Wrong index fort teleport to chain ${receiveChainId}`
      );
      tokenAmount1 -= sendAmount;
    });
    ...
    it("should succeed a teleport from the second chain", async () => {
      ...
      assert.equal(
        await instance.indexes(secondReceiveChainId),
        lastChain2Index,
        `Wrong index fort teleport to chain ${secondReceiveChainId}`
      );
      tokenAmount1 -= sendAmount;
    });
  });
  describe("Check freeze", function () {
    ...
    const logDataTry = {
      ...
      from: "wololo",
      quantity: `${fullToken1}.${"0".repeat(TestSettings.Token.decimals)} ${
        TestSettings.Token.symbol
      }`,
    ...
    it("should fail with unauthorized accound", async () => {
      await catchRevert(
        instance.freeze(true, false, false, { from: accounts[3] }),
        "Unauthorized execution of freeze function"
      );
    });
    it("shoudl succeed to freeze claim function ", async () => {
      await instance.freeze(true, false, false, { from: accounts[0] });
      assert.equal(
        await instance.freezedClaim.call(),
        true,
        "Claim is not freezed"
      );
      assert.equal(
        await instance.freezedTeleport.call(),
        false,
        "Teleport is freezed"
      );
      assert.equal(
        await instance.freezedTransfer.call(),
        false,
        "Transfer is freezed"
      );
      await catchRevert(
        instance.claim(logDataHex, signatures, { from: accounts[2] }),
        "Can claim even it is freezed"
      );
    });
    it("should succeed to freeze teleport function ", async () => {
      ...
      await catchRevert(
        instance.teleport(
          "frauerdbeere",
          sendAmount,
          TestSettings.chainId + 2,
          { from: accounts[1] }
        ),
        "Can teleport even it is freezed"
      );
    });
    it("should succeed to freeze transfer function ", async () => {
      ...
      assert.equal(
        await instance.freezedTransfer.call(),
        true,
        "Transfer not freezed"
      );
      await catchRevert(
        instance.transfer(accounts[1], sendAmount, { from: accounts[1] }),
        "Can tranfer even it is freezed"
      );
    });
  ...
  describe("Set minimum teleport amount", function () {
    ...
    it("should fail with unaothorized account", async () => {
      await catchRevert(
        instance.setMin(minAmount, { from: accounts[3] }),
        "Unauthorized set min"
      );
    });
    it("should succeed to set min", async () => {
      await instance.setMin(minAmount, { from: accounts[0] });
      assert.equal(
        (await instance.minteleport.call()).valueOf(),
        minAmount,
        "Minimum teleport amount was not setted correctly"
      );
    });
    it("should fail a teleport with lower amount than min amount", async () => {
      await catchRevert(
        instance.teleport("fraugertrud", 10, receiveChainId, {
          from: accounts[1],
        }),
        "Transaction with less amount than min amount"
      );
    });
    it("should succeed a teleport with min amount", async () => {
      await instance.teleport("fraugertrud", minAmount, receiveChainId, {
        from: accounts[1],
      });
      assert.equal(
        (await instance.balanceOf.call(accounts[1])).valueOf(),
  ...
  describe("Update threshold", function () {
    it("should fail with unaothorized account", async () => {
      await catchRevert(
        instance.updateThreshold(2, { from: accounts[3] }),
        "Unauthorized update"
      );
    });
    it("should succeed to lower the threshold", async () => {
      ...
      assert.equal(
        Number((await instance.threshold.call()).valueOf()).valueOf(),
        2,
    ...
    it("should fail with same value", async () => {
      await catchRevert(
        instance.updateThreshold(2, { from: accounts[0] }),
        "Update to the same value"
      );
    });
    it("should fail to use values over 10", async () => {
      await catchRevert(
        instance.updateThreshold(11, { from: accounts[0] }),
        "Update a value over 10"
      );
    });
    it("should fail to use too low values", async () => {
      await catchRevert(
        instance.updateThreshold(0, { from: accounts[0] }),
        "Update a value lower than 1"
      );
    });
    it("should succeed to make the threshold higher", async () => {
      await instance.updateThreshold(4, { from: accounts[0] });
      assert.equal(
        Number((await instance.threshold.call()).valueOf()),
        4,
        "Threshold is not updated"
      );
    });
  ...
  describe("Update chain id", function () {
    it("should fail with unaothorized account", async () => {
      await catchRevert(
        instance.updateChainId(3, { from: accounts[3] }),
        "Unauthorized update"
      );
    });
    it("should succeed to lower the chain id", async () => {
      ...
      assert.equal(
        Number((await instance.thisChainId.call()).valueOf()).valueOf(),
        1,
        "Chain id is not updated"
      );
    });
    it("should fail with same value", async () => {
      await catchRevert(
        instance.updateChainId(1, { from: accounts[0] }),
        "Update to the same value"
      );
    });
    it("should fail to use values over 255", async () => {
      await catchRevert(
        instance.updateChainId(255, { from: accounts[0] }),
        "Update a value over 255"
      );
    });
    it("should fail to use too low values", async () => {
      await catchRevert(
        instance.updateChainId(0, { from: accounts[0] }),
        "Update a value lower than 1"
      );
    });
    it("should succeed to make the chain id higher", async () => {
      ...
      assert.equal(
        Number((await instance.thisChainId.call()).valueOf()),
        2,
        "Threshold is not updated"
      );
    });
  ...
  describe("Transfer ownership", function () {
    ...
    it("should succeed to check current ownership", async () => {
      ...
      assert.equal(
        owner,
        accounts[0],
        "NewOwner is at the beginning not account 0"
      );
    });
    it("should fail with unaothorized account", async () => {
      await catchRevert(
        instance.transferOwnership(accounts[5], { from: accounts[3] }),
        "Unauthorized transferOwnership"
      );
    });
    it("should succeed to set new ownership", async () => {
      ...
      assert.equal(
        owner,
        accounts[0],
        "TransferOwnership but owner should still be account 0"
      );
      assert.equal(
        newOwner,
        accounts[5],
        "TransferOwnership but new owner is not account 5"
      );
    });
    it("should fail to accept ownership by unaothorized accounts", async () => {
      await catchRevert(
        instance.acceptOwnership({ from: accounts[3] }),
        "Unauthorized acceptOwnership"
      );
      await catchRevert(
        instance.acceptOwnership({ from: accounts[0] }),
        "Unauthorized acceptOwnership by old owner"
      );
    });
    it("should succeed to accept ownership", async () => {
      ...
      assert.equal(
        owner,
        accounts[5],
        "AcceptOwnership the new owner is not account 5"
      );
      assert.equal(
        newOwner,
        "0x0000000000000000000000000000000000000000",
        "The newOwner variable must be setted to 0x00 address"
      );
    });
  });
});
