...
type EthDataConfig = {
    precision: number;
    symbol: string;
    eos: { oracleAccount: string; id?: number; netId: string };
};
...
class EthOracle {
     ...
     * @returns
     */
    static extractEthClaimedData(
        version: number,
        data: ethers.utils.Result,
        config: EthDataConfig
    ): eosio_claim_data | false {
        if (version >= 2) {
            ...
            if (config.eos.netId.substring(0, 8) != chainNet) {
                console.log(
                    `Found teleport with other net id ${chainNet} is not queal to ${config.eos.netId.substring(0, 8)}`
                );
                return false;
            }
     ...
     * @returns
     */
    static extractEthTeleportData(
        version: number,
        data: ethers.utils.Result,
        transactionHash: string,
        config: EthDataConfig
    ): eosio_teleport_data | false {
        ...
        if (version >= 2) {
            ...
            const { chain_id, id, quantity, amount } = EthOracle.getLogTelParams(
                combiparam.substring(2).padStart(64, '0')
            );
            if (chain_id != config.eos.id) {
                console.log('Wrong chain id', chain_id);
                return false;
                    ...
                    if (overConfs > 0) {
                        ...
                        if (overConfs > 1) {
                            if (firstEp == undefined) {
                                ...
                                console.error(
                                    `Verification failed, only ${validators.size} eth endpoints verified the transaction ${entry.transactionHash} ❌`
                                );
                                return false;
                            }
    ...
    async process_claimed(from_block: number, to_block: number, trxBroadcast: boolean = true) {
        ...
        for await (const entry of res) {
            ...
            if (entry.removed) {
                this.telegram.logError(
                    `Claimed event with trx hash ${TgM.sToMd(
                        entry.transactionHash
                    )} got removed and will be skipped by ${TgM.sToMd(this.config.eos.oracleAccount)} on ${TgM.sToMd(
                        this.config.eth.network
                    )} ❌`,
                    true,
                    true
                );
            ...
            if (eos_res === false) {
                this.telegram.logError(
                    `Skip sending claimed of id ${TgM.sToMd(eosioData.id.toString())} to eosio chain by ${TgM.sToMd(
                        this.config.eos.oracleAccount
                    )} from ${TgM.sToMd(this.config.eth.network)} ❌`,
                    true,
                    true
                );
            } else if (eos_res === true) {
                console.log(
                    `Id ${eosioData.id} is already claimed, account 0x${eosioData.to_eth.substring(0, 40)}, quantity ${
                        eosioData.quantity
                    } ✔️`
                );
            } else {
                console.log(
                    `Send claimed of id ${eosioData.id}, account 0x${eosioData.to_eth.substring(0, 40)}, quantity ${
                        eosioData.quantity
        ...
        for (let tries = 0; tries < this.minTrySend; tries++) {
            try {
                ...
                if (e.message) {
                    ...
                    if (
                        error.indexOf('Already marked as claimed') > -1 ||
                ...
                console.error(
                    `Error while sending transaction to ${actions.length > 0 ? actions[0].account : 'unkown'} on ${
                        this.config.eos.network
                    } chain with ${this.eos_api.getEndpoint()}: ${error} ❌\n${String(e)}`
                );
    ...
    async process_teleported(from_block: number, to_block: number, trxBroadcast: boolean = true) {
        ...
        for await (const entry of res) {
            ...
            const eosioData = EthOracle.extractEthTeleportData(
                this.version,
                decodedData,
                entry.transactionHash,
                this.config
            ...
            if (this.config.eos.id !== undefined && eosioData.chain_id !== Number(this.config.eos.id)) {
                console.log(
                    `Skip teleport event with ${eosioData.to} as recipient and ref of ${eosioData.ref} because the chain id ${eosioData.chain_id} referes to another blockchain.`
                );
                continue;
            }
            ...
            if (eos_res === false) {
                this.telegram.logError(
                    `*${TgM.sToMd(this.config.eos.oracleAccount)}* on *${TgM.sToMd(
                        this.config.eos.network
                    )}* skips sending teleport to ${TgM.sToMd(eosioData.to)} with ref ${TgM.sToMd(
                        eosioData.ref
                    )} and quantity of ${TgM.sToMd(eosioData.quantity)} ❌`,
                    true,
                    true
                );
            } else if (eos_res === true) {
                console.log(
                    `Oracle has already approved teleport to ${eosioData.to} with ref ${eosioData.ref} and quantity of ${eosioData.quantity} ✔️`
                );
            } else {
                console.log(
                    `Send teleport to ${eosioData.to} with ref ${eosioData.ref} and quantity of ${eosioData.quantity}. EOSIO blocknumber ${eos_res.processed.block_num} and trx id ${eos_res.transaction_id} ✔️`
                );
            }
        }
    }
    ...
    async run(start_ref: 'latest' | number | undefined, trxBroadcast: boolean = true, waitCycle = 30) {
        this.telegram.logViaBot(
            `Starting *${TgM.sToMd(this.config.eth.network)}* oracle with *${TgM.sToMd(
                this.config.eos.oracleAccount
            )}* and ${TgM.sToMd(this.config.eth.oracleAccount)} 🏃`,
            true,
            true
        );
        ...
        try {
            ...
            await this.telegram.logError(
                `⚡️ by ${this.config.eos.oracleAccount} on ${this.config.eth.network} \n${String(e)}`
            );
        }
        await this.telegram.logViaBot(
            `Thread closed of *${TgM.sToMd(this.config.eth.network)}* oracle with *${TgM.sToMd(
                this.config.eos.oracleAccount
            )}* and ${TgM.sToMd(this.config.eth.oracleAccount)} 💀`,
            true,
            true
