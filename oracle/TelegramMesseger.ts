...
export class TgM {
    ...
    static sToMd = (str: string) => {
        return TgM.rpMarkDown.reduce((str, replacement) => {
    ...
    constructor(config_telegram: Telegram | undefined) {
        if (config_telegram) {
            if (typeof config_telegram.statusIds != 'object') {
                console.error(
                    'Use the telegram id provider to get you personal contactId and store it in the config file'
                );
                process.exit(1);
            } else {
            ...
            this.telegram.bot = new TelegramBot(config_telegram.privateToken, {
                polling: false,
            });
        }
    }
    ...
    public async logViaBot(msg: string, markdown: boolean = false, no_convert?: boolean) {
        ...
        if (this.telegram.bot) {
            for (let id of this.telegram.statusIds) {
                await this.telegram.bot.sendMessage(id, no_convert === true ? msg : TgM.sToMd(msg), {
