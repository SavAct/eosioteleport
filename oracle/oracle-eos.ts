...
class EosOracle {
    ...
    constructor(private config: ConfigType, private signatureProvider: JsSignatureProvider, private force: boolean) {
        ...
        this.eosio_data = {
            ...
            short_net_id: fromHexString(config.eos.netId.substring(0, 8)),
        };
    }
            ...
            if (tries < this.config.eos.endpoints.length && retry) {
                ...
                if (tooManyFailed) {
                    ...
                    this.telegram.logError(
                        `Teleport id ${id.toString()}, skip sign action by ${this.config.eos.oracleAccount} on ${
     ...
     * @returns Serialized data as Uint8Array
     */
    static serializeLogData_v2(
        eosio_chain_data: EOSIO_Chain_Data,
        teleport: {
            id: number;
            time: number;
            account: string;
    ...
    async getNextTeleports(request: { lowerId: number; amount: number }) {
        ...
        if (lowest_amount > 0) {
            ...
            for (let i = 1; i < this.config.eos.epVerifications; i++) {
                await this.eos_api.nextEndpoint();
                const vData = ((await this.getTableRows(request.lowerId, request.amount, false)) as GetTableRowsResult)
    ...
    async signAllTeleportsUntilNow(signProcessData: { lowerId: number; amount: number }) {
        ...
        for (let rowIndex = 0; rowIndex < lowest_amount; rowIndex++) {
            ...
            if (!isVerifyed) {
                this.telegram.logError(
                    `Teleport id ${TgM.sToMd(item.id.toString())}, skip this one by *${TgM.sToMd(
                        this.config.eos.oracleAccount
                    )}* on *${TgM.sToMd(this.config.eos.network)}* ❌`,
                    true,
                    true
                );
            } else {
    ...
    async run(id: undefined | number = undefined, requestAmount = 100, waitCycle = EosOracle.maxWait) {
        ...
        await this.telegram.logViaBot(
            `Starting *${TgM.sToMd(this.config.eos.network)}* oracle with *${TgM.sToMd(
                this.config.eos.oracleAccount
            )}* beginning by *id ${TgM.sToMd(id.toString())}* 🏃`,
            true,
            true
        );
        ...
        try {
            ...
            await this.telegram.logError(
                `⚡️ by ${this.config.eos.oracleAccount} on ${this.config.eos.network} \n${String(e)}`
            );
        }
        await this.telegram.logViaBot(
            `Thread closed of *${TgM.sToMd(this.config.eos.network)}* oracle with *${TgM.sToMd(
                this.config.eos.oracleAccount
            )}* 💀`,
            true,
            true
